...
public class OrderService {
	...
	public OrderDTO placeOrder(OrderAddressRequestDTO orderRequest) {
		...
		Cart activeCart = cartRepository.findByUserAndIsActive(user, true).stream().findFirst()
		...
		for (CartItems cartItem : cartItems) {
			...
			if (inventory.getAvailableStock() < cartItem.getQuantity()) {
				throw new RuntimeException("Insufficient stock for product: " + product.getProductName()
						+ ". Available: " + inventory.getAvailableStock() + ", Requested: " + cartItem.getQuantity());
			}
...
//      }
			...
			double price = sellingPrice != null ? sellingPrice : (mrp != null ? mrp : 0.0);
		...
		if (cartPayableAmount != null && Math.abs(cartPayableAmount - grandTotal) > 1.0) {
			log.warn("Cart payable amount (${}) differs from calculated amount (${})", cartPayableAmount, grandTotal);
		}
		...
		for (Inventory inventory : inventoriesToReserve) {
			...
			try {
				...
				log.info("Stock reserved for product {}: {} units. Available: {}, Reserved: {}",
						inventory.getProduct().getProductName(), quantity, inventory.getAvailableStock() - quantity,
		...
		log.info("Order placed successfully. Order ID: {}, Amount: {}, Tax: {}, Discount: {}", savedOrder.getId(),
				grandTotal, totalTax, totalDiscount);
		// savedOrder.setOrderStatus(OrderStatus.PENDING);
		orderRepository.save(savedOrder);

		try {
			ShiprocketCreateOrderResponse shiprocketResponse = shiprocketService.createOrder(savedOrder, user,
					selectedAddress);
			...
			if (shiprocketResponse.getAwbCode() != null) {
				...
				savedOrder
						.setShiprocketTrackingUrl("https://shiprocket.co/tracking/" + shiprocketResponse.getAwbCode());
			}
	...
	private Cart getActiveCartWithItems(Long userId) {
		...
		List<Cart> activeCarts = cartRepository.findByUserAndIsActive(userRepository.findById(userId).orElse(null),
				true);
	...
	public List<Map<String, Object>> getShippingRates(Long orderId) {
		...
		return shiprocketService.getShippingRates(pickupAddress, deliveryAddress, totalWeight, 10, 10, 10);
	}
...
//                }
				...
				if (product.getImages() != null && !product.getImages().isEmpty()) {
					...
					if (firstImage != null && firstImage.getImageUrl() != null) {
						try {
							...
							log.warn("Failed to get image URL for product {}: {}", product.getId(), e.getMessage());
	...
	public Map<String, Object> getOrderStatistics() {
		...
		long confirmedOrders = allOrders.stream().filter(o -> o.getOrderStatus() == OrderStatus.CONFIRMED).count();
		long shippedOrders = allOrders.stream().filter(o -> o.getOrderStatus() == OrderStatus.SHIPPED).count();
		long deliveredOrders = allOrders.stream().filter(o -> o.getOrderStatus() == OrderStatus.DELIVERED).count();

		double totalRevenue = allOrders.stream().filter(o -> o.getOrderStatus() == OrderStatus.DELIVERED)
				.mapToDouble(Order::getOrderAmount).sum();

		return Map.of("totalOrders", totalOrders, "pendingOrders", pendingOrders, "confirmedOrders", confirmedOrders,
				"shippedOrders", shippedOrders, "deliveredOrders", deliveredOrders, "totalRevenue", totalRevenue);
	}
	...
	private String formatAddress(Address address) {
		...
		if (address.getApartmentName() != null)
			sb.append(address.getApartmentName()).append(", ");
	...
	public List<TopOrderedProductsDTO> getTopOrderedProductsLast30Days() {
		...
		List<TopOrderedProductsDTO> results = orderItemsRepository.findTopOrderedProductsByDateRange(startDate,
				endDate);
